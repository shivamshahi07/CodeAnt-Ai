...
const StackedCards = () => {
  return (
    <div
      className={`hidden md:flex h-full w-1/2 bg-white  relative flex-col justify-center items-center`}
    >
      <div
        className={`flex rounded-xl shadowCardLogin flex-col w-full max-w-[400px]`}
      >
        <div className={`flex gap-2 items-center border-b p-4`}>
          <img src={"/Subtract.png"} alt={"logo"} className={`w-6 h-6`} />
      ...
      <div
        className={`flex rounded-xl shadowCardLogin flex-col gap-2 p-4 px-8 translate-x-[50%] -translate-y-[10px] bg-white`}
      >
        <div className={`flex justify-between min-w-40`}>
          <img src={"/GraphAuth.svg"} alt={"graph"} className={``} />
          <div className={`flex flex-col`}>
            <div className={`flex items-center text-primary`}>
              <ArrowUp className={`w-5 text-blue-600`} />
              14%
            </div>
